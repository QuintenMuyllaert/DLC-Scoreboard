cp -R -T modules/ ./packages/dlcscoreboard/auth/modules/
cp -R -T modules/ ./packages/dlcscoreboard/status/modules/
cp -R -T modules/ ./packages/dlcscoreboard/readtest/modules/
cp -R -T modules/ ./packages/dlcscoreboard/register/modules/
cp -R -T modules/ ./packages/dlcscoreboard/revoke/modules/
cp -R -T modules/ ./packages/dlcscoreboard/logout/modules/
cp -R -T modules/ ./packages/dlcscoreboard/changepassword/modules/